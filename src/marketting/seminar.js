...
const uploadSeminarFile = async (file, setFile, fetchSeminars) => {
  if (!file) return alert("Select a file first!");
  const formData = new FormData();
  formData.append("file", file);

  try {
    await axios.post("https://web-production-5b5db.up.railway.app/api/seminar-upload/", formData, {
...
// --- React Component ---
export default function SeminarTracker() {
        ...
        <div className="form-box">
          <div className="form-container">
            ...
            <div style={{ marginTop: "10px", display:"flex", gap:"10px", alignItems:"center" }}>
              ...
              <button onClick={() => uploadSeminarFile(file, setFile, () => fetchSeminars(setSeminars))} style={{ background:"#27ae60", color:"#fff", border:"none", padding:"6px 12px", borderRadius:"4px", cursor:"pointer" }}>Upload File</button>
              <button onClick={handleAdd} style={{ background:"#3498db", color:"#fff", border:"none", padding:"6px 12px", borderRadius:"4px", cursor:"pointer" }}>Add to Table</button>
            </div>
          </div>
        </div>
