...
const Dashboard = () => {
  ...
  useEffect(() => {
    fetchDashboardData();
  }, [month, financialYear]); // Add financialYear to dependencies
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Since Walkin doesn't have created_at, count all walkins as current month
      ...
      // Calculate collection from registrations
      ...
      // Calculate GST (18% of collection)
  ...
  // Sample company growth data for chart
  ...
  // Pie chart data for growth percentage
  const pieChartData = {
